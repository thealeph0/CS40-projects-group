...
public class DebugEight1
{
   public static void main(String args[])
   {
      ...
      boolean found = false;
      char[] okayCodes = {'A','C','T','H'}; // comma separation
      StringBuffer prompt = new 
            StringBuffer("Enter shipping code for this delivery\nValid codes are: ");
      for(int x = 0; x < okayCodes.length; ++x)  //okayCodes.length
      {
      ...
      for(int i = 0; i < okayCodes.length; ++i) //okayCodes.length
      {
         if(userCode == okayCodes[i]) //is it identical to the i-th element of okayCodes array
         {
            found = true;
         }
      }
