...
public class DebugNine3
{
   public static void main(String[] args)
   {
      ...
      display(products); //product's' with an s
      final String QUIT = "quit";
      String entry;
      Scanner input = new Scanner(System.in);
      System.out.print("\nEnter a product or "  + QUIT +  " to quit >> "); //QUIT is a variable
      entry = input.nextLine();
      while(!entry.equals("quit"))
      {
         products.add(entry);
         Collections.sort(products);
         display(products); //closing semi
   ...
   public static void display(ArrayList products)
   {
      ...
      for(int x = 0; x < products.size(); ++x) //products.size
